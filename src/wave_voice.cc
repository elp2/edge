...
WaveVoice::~WaveVoice() { delete sound_buffer_; }
...
void WaveVoice::GenerateSoundBuffer() {
  ...
  int fi = 0;
  while (i <
         std::min(int(sound_length * SAMPLE_RATE), MAX_SOUND_BUFFER_LENGTH)) {
    uint8_t val = fi % 2 == 0 ? NIBBLEHIGH(wave_pattern_[fi / 2])
                              : NIBBLELOW(wave_pattern_[fi / 2]);
