...
void TimerController::SetByteAt(uint16_t address, uint8_t byte) {
  ...
  // cout << "Timer updated  " << hex << unsigned(address) << " = " << hex <<
  // unsigned(byte); cout << " Active: " << active_ << " TIMA : " << hex <<
  // unsigned(tima_); cout << " div:" << hex << unsigned(div_counter_) << " APC:
  // " << advance_per_cycle_ << endl;
}
