...
TEST(BitCommandsTest, RLCA) {
  ...
  // Examples say 0x0A but that makes no sense, since the bit7 is 1 and should
  // be in bit0. The logo works when it's like this, so gonna stick with this.
...
TEST(BitCommandsTest, CP2) {
  ...
  CPU *cpu =
      getTestingCPUWithInstructions(vector<uint8_t>{CP_B, CP_N, 0x3C, CP_HLA});
